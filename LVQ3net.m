function [ weight,output ] = LVQ3net( trainX, trainY, trainRate, iteration, inWeight )
%LVQ3NET Summary of this function goes here
...
for count=1:iteration
    for train=1:nTrain
        ...
        if(output(train,mnId(1))~=output(train,mnId(2)))
            if mn(1)/mn(2)> (1-tolerance)*(1+tolerance)
                ...
                elseif(trainY(train,mnId(1))==output(train,mnId(1)))
                    weight(:,mnId(1))=weight(:,mnId(1))+trainRate(count)*(trainX(train,:)'-weight(:,mnId(1)));
                    weight(:,mnId(2))=weight(:,mnId(2))-trainRate(count)*(trainX(train,:)'-weight(:,mnId(2)));
                elseif(trainY(train,mnId(1))==output(train,mnId(1))) && (trainY(train,mnId(1))==output(train,mnId(2)))
                    weight(:,mnId(1))=weight(:,mnId(1))+rateMultiplier*trainRate(count)*(trainX(train,:)'-weight(:,mnId(1)));
                    weight(:,mnId(2))=weight(:,mnId(2))+rateMultiplier*trainRate(count)*(trainX(train,:)'-weight(:,mnId(2)));
                end
            end
        end
        output = evaluateNet(weight, trainX,'distance');
